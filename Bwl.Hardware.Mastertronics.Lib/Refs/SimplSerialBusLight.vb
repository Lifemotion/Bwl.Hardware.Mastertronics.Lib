...
''' </summary>
''' <remarks></remarks>
Public Class SimplSerialBusLight
    Public Class Tests
        Public Shared Sub AddressSetupTest(sserial As SimplSerialBusLight)
    ...
    Private _serial As IO.Ports.SerialPort
    ...
    Public Sub New(serial As IO.Ports.SerialPort)
        _serial = serial
    End Sub

    Public Sub New()
        _serial = New IO.Ports.SerialPort
    End Sub
    ...
    Public Sub Send(request As SSRequest)
        ...
        _serial.Write(bytes.ToArray, 0, bytes.Length)
    ...
    Public Function Request(requestPacket As SSRequest) As SSResponse
        SyncLock _syncRoot
            Dim result As New SSResponse
            SyncLock _serial
                'Не менять на ReadExisting, на линуксе виснет
